...
export default function QuizView({ topicId }: { topicId: number | null }) {
  ...
  if (!topicId) {
    return <div className="p-8 text-sm text-stone-500">Select a topic to take its quiz.</div>;
  }

  return (
    <div className="p-8 max-w-2xl mx-auto space-y-5">
      <h2 className="text-lg font-semibold text-[#222222]">Quiz — {topic?.name}</h2>

      {loading && <p className="text-sm text-stone-500">Loading quiz...</p>}
      ...
      {result && (
        <div className="surface rounded-xl p-4 text-sm text-[#222222] border border-[#FF6D1F]/30">
